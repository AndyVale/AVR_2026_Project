...
namespace airlib
{
    ...
    public: //types
        ...
        static constexpr char const* kSimModeTypeMultidomain = "Multidomain";
    ...
    private:
        ...
        void loadCoreSimModeSettings(const Settings& settings_json, std::function<std::string(void)> simmode_getter)
        {
            ...
            if (physics_engine_name == "") {
                if (simmode_name == kSimModeTypeMultirotor || simmode_name == kSimModeTypeMultidomain)
                                         ...
                                         const std::map<std::string, std::shared_ptr<SensorSetting>>& sensor_defaults)
        {
            ...
            else if (simmode_name == kSimModeTypeMultidomain) {
                // create a default Car (PhysXCar)
                auto physx_car_setting = std::unique_ptr<VehicleSetting>(new VehicleSetting("DefaultCar", kVehicleTypePhysXCar));
                physx_car_setting->sensors = sensor_defaults;
                vehicles[physx_car_setting->vehicle_name] = std::move(physx_car_setting);

                // create a default Multirotor (SimpleFlight)
                auto simple_flight_setting = std::unique_ptr<VehicleSetting>(new VehicleSetting("DefaultDrone", kVehicleTypeSimpleFlight));
                simple_flight_setting->rc.remote_control_id = 0;
                simple_flight_setting->sensors = sensor_defaults;
                vehicles[simple_flight_setting->vehicle_name] = std::move(simple_flight_setting);
                                              ...
                                              CameraDirectorSetting& camera_director, const std::string& simmode_name)
        {
            ...
            if (std::isnan(camera_director.follow_distance)) {
                if (simmode_name == kSimModeTypeCar || simmode_name == kSimModeTypeMultidomain)
