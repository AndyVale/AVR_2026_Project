...
#include "SimModeWorldMultidomain.h"
...
void ASimModeWorldMultidomain::BeginPlay()
{
    Super::BeginPlay();
...
void ASimModeWorldMultidomain::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void ASimModeWorldMultidomain::setupClockSpeed()
{
...
std::vector<std::unique_ptr<msr::airlib::ApiServerBase>> ASimModeWorldMultidomain::createApiServer() const
{
...
void ASimModeWorldMultidomain::getExistingVehiclePawns(TArray<AActor*>& pawns) const
{
    ...
    {
        pawns.Add(fpawn);
        if (getSettings().simmode_name == "Multidomain") {
...
bool ASimModeWorldMultidomain::isVehicleTypeSupported(const std::string& vehicle_type) const
{
    return ((vehicle_type == AirSimSettings::kVehicleTypeSimpleFlight) ||
        (vehicle_type == AirSimSettings::kVehicleTypePhysXCar));
}
std::string ASimModeWorldMultidomain::getVehiclePawnPathName(const AirSimSettings::VehicleSetting& vehicle_setting) const
{
...
PawnEvents* ASimModeWorldMultidomain::getVehiclePawnEvents(APawn* pawn) const
{
...
const common_utils::UniqueValueMap<std::string, APIPCamera*> ASimModeWorldMultidomain::getVehiclePawnCameras(
    APawn* pawn) const
{
    std::string vehicle_type = getVehicleType(pawn);
    if (vehicle_type == AirSimSettings::kVehicleTypePhysXCar) {
...
void ASimModeWorldMultidomain::initializeVehiclePawn(APawn* pawn)
{
    std::string vehicle_type = getVehicleType(pawn);
    if (vehicle_type == AirSimSettings::kVehicleTypePhysXCar) {
...
std::unique_ptr<PawnSimApi> ASimModeWorldMultidomain::createVehicleSimApi(
...
msr::airlib::VehicleApiBase* ASimModeWorldMultidomain::getVehicleApi(const PawnSimApi::Params& pawn_sim_api_params,
