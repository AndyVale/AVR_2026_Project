...
#include "Vehicles/Multidomain/SimModeWorldMultidomain.h"
...
std::string ASimHUD::getSimModeFromUser()
{
    if (EAppReturnType::No == UAirBlueprintLib::ShowMessage(EAppMsgType::YesNo,
                                                            "Would you like to use single-vehicle or multi-vehicle simulation? Choose no to use multi-vehicle simulation.",
                                                            "Choose Vehicle")) {
        return AirSimSettings::kSimModeTypeMultidomain;
    }
    else
        if (EAppReturnType::No == UAirBlueprintLib::ShowMessage(EAppMsgType::YesNo,
...
void ASimHUD::createSimMode()
{
    ...
    if (simmode_name == AirSimSettings::kSimModeTypeMultirotor)
        ...
        simmode_ = this->GetWorld()->SpawnActor<ASimModeComputerVision>(FVector::ZeroVector,
                                                                        FRotator::ZeroRotator,
                                                                        simmode_spawn_params);
    else if (simmode_name == AirSimSettings::kSimModeTypeMultidomain)
        simmode_ = this->GetWorld()->SpawnActor<ASimModeWorldMultidomain>(FVector::ZeroVector,
            FRotator::ZeroRotator,
            simmode_spawn_params);
    else {
